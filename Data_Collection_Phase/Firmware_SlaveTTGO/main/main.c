...
static void sys_monitor_task(void *pvParameters) {
    ...
    while (1) {
        ...
        // 本地螢幕除錯輸出 (使用 %zu 匹配 size_t 避免 v5.5 報錯)
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "Initializing SlaveTTGO (Node %d)...", SLAVE_NODE_ID);
    ...
    // 3. 初始化 ESP-NOW 發送模組 (Unicast 給 Logger)
    espnow_tx_init();
    ...
    // 7. 啟動檔案管理大腦 (計算絕對時間、CSV 寫入、事件回報、控制 GPIO 21)
    app_file_manager_start(data_sync_queue);

    // 8. 啟動系統監控與心跳任務 (優先權最低)
    xTaskCreate(sys_monitor_task, "sys_monitor", 4096, NULL, 5, NULL);

    ESP_LOGI(TAG, "SlaveTTGO fully started. Waiting for Scanner data and GPS fix...");
}