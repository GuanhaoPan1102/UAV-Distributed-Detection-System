...
#include "app_route_mapper.h"   // [新增] 讓 Dashboard 能餵資料給 Mapper

static const char *TAG = "LOGGER_DASH";

/**
 * @brief 將 Unix Timestamp 轉換為格式化字串 (HH:MM:SS)
 *        Slave 已做 +8h 偏移，這裡用 gmtime 避免二次偏移。
 */
...
/**
 * @brief 處理並列印所有來自節點的事件
 *
 *  與舊版的唯一差異：MSG_TYPE_FILE_SAVED case 新增一行
 *  app_route_mapper_feed() 呼叫，其餘邏輯完全不變。
 */
    ...
    switch (msg->msg_type) {
        ...
        case MSG_TYPE_UAV_EVENT:
            if (msg->data.uav_event.event_type == 1) {
                printf("\033[33m[WARN]\033[0m Node %02d: \033[1mDISCOVER\033[0m "
        ...
        case MSG_TYPE_FILE_SAVED:
            ...
            // ── [新增] 錄製中時，將此筆存檔資訊餵給 Route Mapper（自動去重）──
            app_route_mapper_feed(msg->node_id, msg->data.file.filename);
            break;
...
/**
 * @brief Dashboard 任務：持續從佇列取出資料並列印
 */
